...
local function get_pitch_lift(y)
    local l = -(1964 / 1755 * y^4) - (2549 / 3510 * y^3) + (2591 / 7020 * y^2) + (2594 / 3510 * y) + 0.75
    return l
end
...
minetest.register_tool("glider:rocket", {
    ...
    on_use = function(itemstack, user, pt)
        ...
        if attach then
            ...
            if luaent.name == "glider:hangglider" then
                luaent.speed = luaent.speed + luaent.time_from_last_rocket + 10  -- Adjust the boost value
                luaent.time_from_last_rocket = 0
                local vert_boost = 5  -- Adjust the vertical boost value
                luaent.grav_speed = luaent.grav_speed + vert_boost
                itemstack:take_item()
                minetest.add_particlespawner({
                    amount = 200, -- Reduced the number of particles
                    time = 2,
